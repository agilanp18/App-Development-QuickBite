...
export default function ShowCart() {
  const { cart, price, incr, quantity, setCart,placeOrder,isLoggedIn,setTotal,total} = useGlobelContext();
  ...
  setTotal(cart.reduce((acc, addedItem) => {
  ...
  return (
    <div className="container-sm border text-center m-2">
      ...
      <button className="mx-2 btn btn-outline-success mb-5" disable={isLoggedIn===false}><Link to={'/checkout'}>Pay Now</Link></button>
      <button className="mx-2 btn btn-success mb-5">Total Prize: {total}</button>
